# 为用到的COCO中图片生成resnet特征
# 训练和测试时直接使用该特征，也就意味着训练过程中不包含编码端
import os
import sys
import json
import pickle
...
for item in dataset_coco:
    if item['split'] != 'train':
        continue
    imgs_coco_train.append(item["filename"])
    
print("Num of train: " + str(len(imgs_coco_train)))
generate_feat(transforms, coco_train_dir, imgs_coco_train, coco_out_dir)
"""
# 准备coco val的40000+图片
coco_train_dir = '/home/data_ti4_c/chengkz/data/coco_dataset/val2014/'
coco_out_dir = '/home/data_ti4_c/chengkz/data/resnet_feat/coco/'
dataset_dir_coco = '/home/data_ti4_c/chengkz/data/caption_datasets/dataset_coco.json'
dataset_coco = json.load(open(dataset_dir_coco, 'r'))['images']
imgs_coco_val = []
for item in dataset_coco:
    if item['split'] == 'train':
        continue
    imgs_coco_val.append(item["filename"])

print("Num of val: " + str(len(imgs_coco_val)))
generate_feat(transforms, coco_train_dir, imgs_coco_val, coco_out_dir)
